...
{
    public class LandFreightPortRepository : IDisposable
    {

        public IList<PortModel> SelectLandFreightPorts()
        {
            try
            {
                using (IDbConnection connection = new SqlConnection(ConnectionHelper.BuildConnectionString()))
                {
                    ...
                    var landFreightPortsRecords =
                        connection.Query<PortModel>(
                            "SELECT * FROM Ports WHERE PortTypeId = 3 AND CompanyId = @CompanyId;", parameters);
                    return landFreightPortsRecords.ToList();
                }
            }
            catch (Exception e)
            {
        ...
        public SeaportModel SelectLandFreightPortById(int landFreightPortId)
        {
            try
            {
                using (IDbConnection connection = new SqlConnection(ConnectionHelper.BuildConnectionString()))
                {
                    var parameters = new DynamicParameters();
                    parameters.Add("@Id", landFreightPortId);

                    var landFreightPortRecord =
                        connection.QuerySingleOrDefault<SeaportModel>("SELECT * FROM Ports WHERE Id = @Id;", parameters);
                    return landFreightPortRecord;
                }
            }
            catch (Exception e)
            {
        ...
        public int AddNewLandFreightPort(PortModel dataModel)
        {
	        try
	        {
		        using (IDbConnection connection = new SqlConnection(ConnectionHelper.BuildConnectionString()))
        ...
        public bool UpdateLandFreightPort(PortModel dataModel)
        {
	        try
	        {
		        using (IDbConnection connection = new SqlConnection(ConnectionHelper.BuildConnectionString()))
        ...
        public bool DeleteLandFreightPort(int id)
        {
            try
            {
                using (IDbConnection connection = new SqlConnection(ConnectionHelper.BuildConnectionString()))
