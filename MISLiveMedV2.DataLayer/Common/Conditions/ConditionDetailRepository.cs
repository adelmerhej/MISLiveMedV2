...
namespace MISLiveMed.DataLayers.Common.Conditions
{
    ...
    {
        public IList<ConditionDetailModel> SelectConditionDetails()
        {
            try
            {
                using (IDbConnection connection = new SqlConnection(ConnectionHelper.BuildConnectionString()))
                {
                    ...
                    var conditionDetailsRecords =
                        connection.Query<ConditionDetailModel>("SELECT * FROM ConditionDetails WHERE CompanyId = @CompanyId;", p);
