...
{
    public partial class AgentGroupEditForm : XtraForm
	{
		private const string _formName = "AgentGroupEdit";
		private int _formId;
		...
		public EventHandler SendUpdatedAgentGroup;
        
        public AgentGroupEditForm(AgentGroupModel agentGroup)
        {
            InitializeComponent();
            
            try
            {
				_agentGroupModel = agentGroup;
        ...
        {
            bsAgentsGroup.DataSource = _agentGroupModel;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                BindingContext[bsAgentsGroup].EndCurrentEdit();

                if (_agentGroupModel.Id == 0)
                {
                    _logInfoRepository.CreateLogInfo(_agentGroupModel);

                    int newContinent = _agentGroupRepository.AddNewAgentGroup(_agentGroupModel);
                    if (newContinent == 0)
                    {
                ...
                {
                    _logInfoRepository.UpdateLogInfo(_agentGroupModel);
                    _agentGroupRepository.UpdateAgentGroup(_agentGroupModel);
                }

                SendUpdatedAgentGroup(_agentGroupModel, EventArgs.Empty);
