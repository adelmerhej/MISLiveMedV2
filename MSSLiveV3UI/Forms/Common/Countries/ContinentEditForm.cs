...
namespace MISLiveMed.UI.Forms.Common.Countries
{
	...
	{
		private const string _formName = "ContinentEdit";
		private int _formId;
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				_continentModel = bsContinent.Current as ContinentModel;

				if (_continentModel == null) return;

				if (_continentModel.Id == 0)
				{
					_logInfoRepository.CreateLogInfo(_continentModel);
					var newCityId = _continentRepository.AddNewContinent(_continentModel);
					txtId.Text = newCityId.ToString();
				}
				else
				{
					_logInfoRepository.UpdateLogInfo(_continentModel);
					_continentRepository.UpdateContinent(_continentModel);
				}
