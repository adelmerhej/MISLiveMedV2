...
{
    public partial class SeaportEditForm : XtraForm
    {
	    private const string _formName = "SeaportEdit";
	    private int _formId;

	    private DataEntityState _objState = DataEntityState.Unchanged;
	    private readonly PortType _portType = PortType.Seaport;
		...
		public EventHandler SendUpdatedSeaport;

        public SeaportEditForm(PortModel model)
        {
            InitializeComponent();
		...
		private void btnSave_Click(object sender, EventArgs e)
        {
	        ...
	        try
	        {
		        BindingContext[bsSeaport].EndCurrentEdit();
		        _seaportModel = bsSeaport.Current as PortModel;

		        if (cboCountries.EditValue != null) _seaportModel.CountryId = (int)cboCountries.EditValue;
		        if (cboCities.EditValue != null) _seaportModel.CityId = (int)cboCities.EditValue;
		        _seaportModel.PortTypeId = (int)_portType;

				if (_seaportModel.Id == 0)
		        {
			        _logInfoRepository.CreateLogInfo(_seaportModel);
			        var newSeaportId = _seaportRepository.AddNewSeaport(_seaportModel);
			        txtId.Text = newSeaportId.ToString();
		        }
		        else
		        {
			        _logInfoRepository.UpdateLogInfo(_seaportModel);
			        _seaportRepository.UpdateSeaport(_seaportModel);
		        }

		        SendUpdatedSeaport(_seaportModel, EventArgs.Empty);
		...
		private void SeaportEditForm_FormClosing(object sender, FormClosingEventArgs e)
		{
