...
{
    public partial class AirportEditForm : XtraForm
    {
	    private const string _formName = "AirportEdit";
	    private int _formId;

	    private DataEntityState _objState = DataEntityState.Unchanged;
	    private readonly PortType _portType = PortType.Airport;
		...
		public EventHandler SendUpdatedAirport;

		public AirportEditForm(PortModel model)
		{
			InitializeComponent();
		...
		private void btnSave_Click(object sender, EventArgs e)
        {
	        ...
	        try
	        {
		        BindingContext[bsAirport].EndCurrentEdit();
		        _airportModel = bsAirport.Current as PortModel;

		        if (cboCountries.EditValue != null) _airportModel.CountryId = (int)cboCountries.EditValue;
		        if (cboCities.EditValue != null) _airportModel.CityId = (int)cboCities.EditValue;
		        _airportModel.PortTypeId = (int)_portType;

				if (_airportModel.Id == 0)
		        {
			        _logInfoRepository.CreateLogInfo(_airportModel);
			        var newAirportId = _airportRepository.AddNewAirport(_airportModel);
			        txtId.Text = newAirportId.ToString();
		        }
		        else
		        {
			        _logInfoRepository.UpdateLogInfo(_airportModel);
			        _airportRepository.UpdateAirport(_airportModel);
		        }

		        SendUpdatedAirport(_airportModel, EventArgs.Empty);
		        Close();
	        }
		...
		private void AirportEditForm_FormClosing(object sender, FormClosingEventArgs e)
		{
