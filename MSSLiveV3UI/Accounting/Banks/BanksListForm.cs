...
namespace MISLiveMed.UI.Accounting.Banks
{
	...
	{
		private const string _formName = "BanksList";
		private int _formId;
		...
		private void InitializeBindings()
		{
			try
			{
				...
				_banks = _bankRepository.SelectBanks();
		...
		{
			BankEditForm frm = new BankEditForm(new BankModel());
			frm.SendUpdatedBank += RcvUpdatedBank;
			frm.ShowDialog();
		}
		...
		private void btnEdit_ItemClick(object sender, ItemClickEventArgs e)
		{
			...
			int currentRowId = (int)gvBanks.GetFocusedRowCellValue("Id");
			if (currentRowId == 0) return;

			_bankModel = _banks.SingleOrDefault(x => x.Id == currentRowId);
			if (_bankModel == null) return;

			var frm = new BankEditForm(_bankModel);
			frm.SendUpdatedBank += RcvUpdatedBank;
			frm.ShowDialog();
		}
		...
		private void btnDelete_ItemClick(object sender, ItemClickEventArgs e)
		{
			...
			try
			{
				int.TryParse(gvBanks.GetFocusedRowCellValue("Id").ToString(), out int id);
				string name = gvBanks.GetFocusedRowCellValue("Name").ToString();
				...
				if (id > 0)
				{
							...
							MessageBoxDefaultButton.Button2) == DialogResult.Yes)
					{
						...
						//delete the record
						if (!_bankRepository.DeleteBank(_bankModel.Id)) return;
						RcvUpdatedBank(_bankModel, EventArgs.Empty);
					}
				}
		...
		#endregion
		private void RcvUpdatedBank(object sender, EventArgs e)
		{
			if (sender == null) return;
			_bankModel = sender as BankModel;

			if (_bankModel != null && (_bankModel.LastModifiedDate == null || _bankModel.Deleted))
			{
				InitializeBindings();
				WireUpBindings();
			}
		...
		private void gcBanks_DoubleClick(object sender, EventArgs e)
		{
			if (!_banks.Any()) return;
