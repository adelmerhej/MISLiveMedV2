...
{
    public partial class AirImportStatusReportForm : XtraForm
	{
		private const string _formName = "AirImportStatusReport";
		private int _formId;
		...
		public AirImportStatusReportForm()
        {
            InitializeComponent();
        ...
        {
	        dtDateFrom.EditValue = DateTime.Parse("01/01/" + DateTime.Now.Year.ToString());
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _dataReportModels =
                    (List<AirDataReportModel>)ReportsDataAccess.GetAirFreightImportReportList((DateTime?)dateFrom,
                        (DateTime?)dateTo, customerId, userId, salesId);

                var previewForm = new DocumentViewerForm();
                var report = new AirImportStatusReport();
                ...
                report.Parameters["ProfitVisibility"].Value = _isAdmin || _canAccessProfit;
