...
{
    public partial class ClientsInvoiceReportForm : XtraForm
	{
		private const string _formName = "ClientsInvoiceReport";
		private int _formId;
		...
		public ClientsInvoiceReportForm()
        {
            InitializeComponent();
        ...
        private void ApplyPermissions()
        {
	        ...
	        chkShowProtected.Visible = _isAdmin;
			chkFilterByCustomDate.Visible = _isAdmin || _canAccessProfit;
		}
		...
		private void ApplyDefaults()
        {
            ...
            chkShowWeekly.Checked = true;
            chkWithAllMembers.Checked = true;
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                bool showWeekly = chkShowWeekly.CheckState == CheckState.Checked;
                ...
                bool showProtected = chkShowProtected.CheckState == CheckState.Checked;

                bool withAllMembers = chkWithAllMembers.CheckState == CheckState.Checked;
                bool filterByCustomDate = chkFilterByCustomDate.CheckState == CheckState.Checked;

                _dataReportModels = _invoiceDetailReportRepository.SelectClientsInvoiceReport(dateFrom, dateTo, userId, customerId, salesId,
                    statusFilter, departmentsFilter, orderBy, sortingBy, fullPaid, withDraft, 
                    withAllMembers, filterByCustomDate, showProtected);

                if (showWeekly)
                {
                    var weeklyReport = new ClientsInvoiceReport();
                    weeklyReport.DataSource = _dataReportModels;

                    weeklyReport.Parameters["DateFrom"].Value = dateFrom;
                    weeklyReport.Parameters["DateTo"].Value = dateTo;
                    weeklyReport.Parameters["ProfitVisibility"].Value = false;
                    weeklyReport.Parameters["ShowWeekly"].Value = showWeekly;

                    previewForm.Viewer.DocumentSource = weeklyReport;
                }

                else
                {
                    var report = new ClientsInvoiceReport();
        ...
        private void statusFilterGroup_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (statusFilterGroup.SelectedIndex)
            {
                case 0:
                    ...
                    statusFilterGroup.Items[1].CheckState = CheckState.Unchecked;
